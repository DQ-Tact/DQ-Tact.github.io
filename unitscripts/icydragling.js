// Set awakening value to 0
var Awk = 0;
// Set elemental rank value to 0
var Elemental = 0;
// List of all stat increases from awakenings
var AwkStatBuff =   {"5":{"hp":10,"mp":0,"atk":0,"def":0,"agl":0,"wis":0,"move":0}
                    ,"4":{"hp":10,"mp":0,"atk":0,"def":0,"agl":0,"wis":0,"move":0}
                    ,"3":{"hp":10,"mp":0,"atk":0,"def":0,"agl":0,"wis":0,"move":0}
                    ,"2":{"hp":10,"mp":0,"atk":0,"def":0,"agl":0,"wis":0,"move":0}
                    ,"1":{"hp":10,"mp":0,"atk":0,"def":0,"agl":0,"wis":0,"move":0}
                    ,"0":{"hp":0,"mp":0,"atk":0,"def":0,"agl":0,"wis":0,"move":0}};
// List of multipliers from stats up awakenings
var AwkStatsUp =    {"5":{"hp":1.25,"mp":1.25,"atk":1.25,"def":1.25,"agl":1.25,"wis":1.25,"move":1},"4":{"hp":1.2,"mp":1.2,"atk":1.2,"def":1.2,"agl":1.2,"wis":1.2,"move":1},"3":{"hp":1.15,"mp":1.15,"atk":1.15,"def":1.15,"agl":1.15,"wis":1.15,"move":1},"2":{"hp":1.1,"mp":1.1,"atk":1.1,"def":1.1,"agl":1.1,"wis":1.1,"move":1},"1":{"hp":1.05,"mp":1.05,"atk":1.05,"def":1.05,"agl":1.05,"wis":1.05,"move":1},"0":{"hp":1,"mp":1,"atk":1,"def":1,"agl":1,"wis":1,"move":1}}; 
// List of multipliers from elemental ranks
var ElementalStatsUp =    {"9":{"hp":0.07,"mp":0.05,"atk":0,"def":0.05,"agl":0,"wis":0,"move":0},"8":{"hp":0.04,"mp":0.04,"atk":0,"def":0.04,"agl":0,"wis":0,"move":0},"7":{"hp":0.04,"mp":0.03,"atk":0,"def":0.03,"agl":0,"wis":0,"move":0},"6":{"hp":0.03,"mp":0.03,"atk":0,"def":0.03,"agl":0,"wis":0,"move":0},"5":{"hp":0.03,"mp":0.02,"atk":0,"def":0.02,"agl":0,"wis":0,"move":0},"4":{"hp":0.02,"mp":0.02,"atk":0,"def":0.02,"agl":0,"wis":0,"move":0},"3":{"hp":0.02,"mp":0.01,"atk":0,"def":0.01,"agl":0,"wis":0,"move":0},"2":{"hp":0.01,"mp":0.01,"atk":0,"def":0.01,"agl":0,"wis":0,"move":0},"1":{"hp":0.01,"mp":0,"atk":0,"def":0,"agl":0,"wis":0,"move":0},"0":{"hp":0,"mp":0,"atk":0,"def":0,"agl":0,"wis":0,"move":0}}; 
// Function to add stat increases from AwkStatBuff onto the unit's base stats before multiplying it with the sum of both AwkStatsUp and ElementalStatsUp
function applyStats() {
    $('#HP').text(Math.floor((434 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].hp : 0)) * ((AwkStatsUp[Awk] ? AwkStatsUp[Awk].hp : 0) + (ElementalStatsUp[Elemental] ? ElementalStatsUp[Elemental].hp : 0))));
    $('#MP').text(Math.floor((149 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].mp : 0)) * ((AwkStatsUp[Awk] ? AwkStatsUp[Awk].mp : 0)+ (ElementalStatsUp[Elemental] ? ElementalStatsUp[Elemental].mp : 0))));
    $('#ATK').text(Math.floor((152 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].atk : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].atk : 0)));
    $('#DEF').text(Math.floor((141 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].def : 0)) * ((AwkStatsUp[Awk] ? AwkStatsUp[Awk].def : 0)+ (ElementalStatsUp[Elemental] ? ElementalStatsUp[Elemental].def : 0))));
    $('#AGL').text(Math.floor((182 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].agl : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].agl : 0)));
    $('#WIS').text(Math.floor((130 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].wis : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].wis : 0)));
    $('#Move').text(4 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].move : 0));
}
// Start of Javascript code: where it checks what has been selected from the drop-down lists on the unit page, and updates the value of Awk and Elemental when the selected option from the drop-down list changes to be the value selected from the drop-down list
$(document).ready( function () {
applyStats();
$(".formawk").on("change", function() {
    Awk = $(this).val();
    applyStats();
});
$(".formelement").on("change", function() {
    Elemental = $(this).val();
    applyStats();
});

} );