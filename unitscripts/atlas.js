...
var TalentBlossomStats1 = {"hp":50,"mp":0,"atk":0,"def":0,"agl":0,"wis":0,"move":0};
var FullTalent1 = false;
var TalentBlossomStats2 = {"hp":50,"mp":0,"atk":0,"def":0,"agl":0,"wis":0,"move":0};
var FullTalent2 = false;
var TalentBlossomStats3 = {"hp":50,"mp":0,"atk":0,"def":0,"agl":0,"wis":0,"move":0};
var FullTalent3 = false;

function applyStats() {
    $('#HP').text(Math.floor((1168 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].hp : 0) + (FullTalent1 ? TalentBlossomStats1.hp : 0) + (FullTalent2 ? TalentBlossomStats2.hp : 0) + (FullTalent3 ? TalentBlossomStats3.hp : 0)) * ((AwkStatsUp[Awk] ? AwkStatsUp[Awk].hp : 0) + (ElementalStatsUp[Elemental] ? ElementalStatsUp[Elemental].hp : 0))));
    $('#MP').text(Math.floor((340 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].mp : 0)) * ((AwkStatsUp[Awk] ? AwkStatsUp[Awk].mp : 0) + (ElementalStatsUp[Elemental] ? ElementalStatsUp[Elemental].mp : 0))));
    $('#ATK').text(Math.floor((527 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].atk : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].atk : 0)));
    $('#DEF').text(Math.floor((374 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].def : 0)) * ((AwkStatsUp[Awk] ? AwkStatsUp[Awk].def : 0) + (ElementalStatsUp[Elemental] ? ElementalStatsUp[Elemental].def : 0))));
    $('#AGL').text(Math.floor((449 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].agl : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].agl : 0)));
    $('#WIS').text(Math.floor((140 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].wis : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].wis : 0)));
