...
var TalentBlossomStats = {"hp":150,"mp":50,"atk":25,"def":25,"agl":25,"wis":25,"move":0};
...
function applyStats() {
    ...
    $('#DEF').text(Math.floor((450 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].def : 0) + (FullTalent ? TalentBlossomStats.def : 0)) * ((AwkStatsUp[Awk] ? AwkStatsUp[Awk].def : 0) + (ElementalStatsUp[Elemental] ? ElementalStatsUp[Elemental].def : 0))));
