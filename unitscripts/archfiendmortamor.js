...
function applyStats() {
    $('#HP').text(Math.floor((1043 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].hp : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].hp : 0)));
    $('#MP').text(Math.floor((439 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].mp : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].mp : 0)));
    $('#ATK').text(Math.floor((209 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].atk : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].atk : 0)));
    $('#DEF').text(Math.floor((407 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].def : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].def : 0)));
