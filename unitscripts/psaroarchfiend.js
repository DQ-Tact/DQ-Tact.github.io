...
function applyStats() {
    $('#HP').text(Math.floor((1609 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].hp : 0)) * ((AwkStatsUp[Awk] ? AwkStatsUp[Awk].hp : 0) + (ElementalStatsUp[Elemental] ? ElementalStatsUp[Elemental].hp : 0))));
    $('#MP').text(Math.floor((540 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].mp : 0)) * ((AwkStatsUp[Awk] ? AwkStatsUp[Awk].mp : 0)+ (ElementalStatsUp[Elemental] ? ElementalStatsUp[Elemental].mp : 0))));
    $('#ATK').text(Math.floor((547 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].atk : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].atk : 0)));
    $('#DEF').text(Math.floor((469 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].def : 0)) * ((AwkStatsUp[Awk] ? AwkStatsUp[Awk].def : 0)+ (ElementalStatsUp[Elemental] ? ElementalStatsUp[Elemental].def : 0))));
    $('#AGL').text(Math.floor((489 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].agl : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].agl : 0)));
    $('#WIS').text(Math.floor((241 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].wis : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].wis : 0)));
...
function changeIcon0() {
    var image = document.getElementById("Switch");
    image.src="../images/PsaroArchfiend1.png";
}
// Function which returns image to initial display
function changeIcon1() {
    var image = document.getElementById("Switch");
    image.src="../images/PsaroArchfiend2.png";
}

function changeIcon2() {
    var image = document.getElementById("Switch");
    image.src="../images/PsaroArchfiend.png";
}
...
$('input:radio[value=0]').change(function() {
    if ($(this).is(":checked")) {
        changeIcon0();
    } else {
        changeIcon2();
...
$('input:radio[value=1]').change(function() {
    if ($(this).is(":checked")) {
        changeIcon1();
    } else {
        changeIcon2();
...
$('input:radio[value=2]').change(function() {
    if ($(this).is(":checked")) {
        changeIcon2();
    } else {
        changeIcon2();
