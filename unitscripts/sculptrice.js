...
function applyStats() {
    $('#HP').text(Math.floor((611 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].hp : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].hp : 0)));
    ...
    $('#DEF').text(Math.floor((296 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].def : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].def : 0)));
