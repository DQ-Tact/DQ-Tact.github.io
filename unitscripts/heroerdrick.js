...
var TalentBlossomStats = {"code":null,"hp":75,"mp":75,"atk":40,"def":90,"agl":40,"wis":40,"move":0,"stype":null};
...
function applyStats() {
    ...
    $('#DEF').text(Math.floor((385 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].def : 0) + (FullTalent ? TalentBlossomStats.def : 0)) * ((AwkStatsUp[Awk] ? AwkStatsUp[Awk].def : 0) + (ElementalStatsUp[Elemental] ? ElementalStatsUp[Elemental].def : 0))));
