...
var AwkStatBuff =   {"5":{"hp":0,"mp":0,"atk":40,"def":0,"agl":0,"wis":0,"move":1}
                    ,"4":{"hp":0,"mp":0,"atk":40,"def":0,"agl":0,"wis":0,"move":1}
                    ,"3":{"hp":0,"mp":0,"atk":40,"def":0,"agl":0,"wis":0,"move":1}
...
function applyStats() {
    $('#HP').text(Math.floor((1202 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].hp : 0)) * ((AwkStatsUp[Awk] ? AwkStatsUp[Awk].hp : 0) + (ElementalStatsUp[Elemental] ? ElementalStatsUp[Elemental].hp : 0))));
    $('#MP').text(Math.floor((390 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].mp : 0)) * ((AwkStatsUp[Awk] ? AwkStatsUp[Awk].mp : 0)+ (ElementalStatsUp[Elemental] ? ElementalStatsUp[Elemental].mp : 0))));
    $('#ATK').text(Math.floor((468 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].atk : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].atk : 0)));
    ...
    $('#AGL').text(Math.floor((434 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].agl : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].agl : 0)));
