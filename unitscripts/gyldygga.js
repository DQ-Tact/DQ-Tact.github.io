...
function applyStats() {
    $('#HP').text(Math.floor((1112 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].hp : 0) + (FullTalent ? TalentBlossomStats.hp : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].hp : 0)));
    $('#MP').text(Math.floor((323 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].mp : 0) + (FullTalent ? TalentBlossomStats.mp : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].mp : 0)));
    ...
    $('#DEF').text(Math.floor((407 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].def : 0) + (FullTalent ? TalentBlossomStats.def : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].def : 0)));
