...
var TalentBlossomStats = {"code":null,"hp":40,"mp":20,"atk":10,"def":20,"agl":70,"wis":10,"move":0,"stype":null};
var TalentBlossomStatsUpNode = {"code":null,"hp":0.4,"mp":0,"atk":0,"def":0,"agl":0,"wis":0,"move":0,"stype":null};
var FullTalent = false;

function applyStats() {
    $('#HP').text(Math.floor((863 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].hp : 0) + (FullTalent ? TalentBlossomStats.hp : 0)) * ((AwkStatsUp[Awk] ? AwkStatsUp[Awk].hp : 0) + (FullTalent ? TalentBlossomStatsUpNode.hp : 0) + (ElementalStatsUp[Elemental] ? ElementalStatsUp[Elemental].hp : 0))));
    $('#MP').text(Math.floor((323 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].mp : 0) + (FullTalent ? TalentBlossomStats.mp : 0)) * ((AwkStatsUp[Awk] ? AwkStatsUp[Awk].mp : 0) + (FullTalent ? TalentBlossomStatsUpNode.mp : 0) + (ElementalStatsUp[Elemental] ? ElementalStatsUp[Elemental].mp : 0))));
    $('#ATK').text(Math.floor((302 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].atk : 0) + (FullTalent ? TalentBlossomStats.atk : 0)) * ((AwkStatsUp[Awk] ? AwkStatsUp[Awk].atk : 0) + (FullTalent ? TalentBlossomStatsUpNode.atk : 0))));
    ...
    $('#WIS').text(Math.floor((216 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].wis : 0) + (FullTalent ? TalentBlossomStats.wis : 0)) * ((AwkStatsUp[Awk] ? AwkStatsUp[Awk].wis : 0) + (FullTalent ? TalentBlossomStatsUpNode.wis : 0))));
