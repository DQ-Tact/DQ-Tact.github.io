...
var AwkStatBuff =   {"5":{"hp":0,"mp":0,"atk":0,"def":0,"agl":10,"wis":10,"move":0}
                    ,"4":{"hp":0,"mp":0,"atk":0,"def":0,"agl":10,"wis":5,"move":0}
                    ,"3":{"hp":0,"mp":0,"atk":0,"def":0,"agl":10,"wis":5,"move":0}
...
function applyStats() {
    $('#HP').text(Math.floor((569 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].hp : 0)) * ((AwkStatsUp[Awk] ? AwkStatsUp[Awk].hp : 0) + (ElementalStatsUp[Elemental] ? ElementalStatsUp[Elemental].hp : 0))));
    ...
    $('#DEF').text(Math.floor((152 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].def : 0)) * ((AwkStatsUp[Awk] ? AwkStatsUp[Awk].def : 0)+ (ElementalStatsUp[Elemental] ? ElementalStatsUp[Elemental].def : 0))));
    $('#AGL').text(Math.floor((183 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].agl : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].agl : 0)));
    $('#WIS').text(Math.floor((158 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].wis : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].wis : 0)));
