...
var AwkStatBuff =   {"5":{"hp":0,"mp":100,"atk":0,"def":0,"agl":0,"wis":0,"move":0}
                    ,"4":{"hp":0,"mp":100,"atk":0,"def":0,"agl":0,"wis":0,"move":0}
                    ,"3":{"hp":0,"mp":100,"atk":0,"def":0,"agl":0,"wis":0,"move":0}
                    ,"2":{"hp":0,"mp":100,"atk":0,"def":0,"agl":0,"wis":0,"move":0}
                    ,"1":{"hp":0,"mp":100,"atk":0,"def":0,"agl":0,"wis":0,"move":0}
...
function applyStats() {
    $('#HP').text(Math.floor((919 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].hp : 0)) * ((AwkStatsUp[Awk] ? AwkStatsUp[Awk].hp : 0) + (ElementalStatsUp[Elemental] ? ElementalStatsUp[Elemental].hp : 0))));
    $('#MP').text(Math.floor((296 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].mp : 0)) * ((AwkStatsUp[Awk] ? AwkStatsUp[Awk].mp : 0)+ (ElementalStatsUp[Elemental] ? ElementalStatsUp[Elemental].mp : 0))));
    $('#ATK').text(Math.floor((245 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].atk : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].atk : 0)));
    ...
    $('#AGL').text(Math.floor((304 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].agl : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].agl : 0)));
    $('#WIS').text(Math.floor((273 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].wis : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].wis : 0)));
