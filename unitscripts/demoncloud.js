...
function applyStats() {
    ...
    $('#MP').text(Math.floor((257 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].mp : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].mp : 0)));
    $('#ATK').text(Math.floor((187 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].atk : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].atk : 0)));
    $('#DEF').text(Math.floor((281 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].def : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].def : 0)));
