...
function applyStats() {
    ...
    $('#MP').text(Math.floor((248 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].mp : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].mp : 0)));
    $('#ATK').text(Math.floor((377 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].atk : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].atk : 0)));
    $('#DEF').text(Math.floor((385 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].def : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].def : 0)));
