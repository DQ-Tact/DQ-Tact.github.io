var Awk = 0;
var Elemental = 0;
var AwkStatBuff =   {"5":{"hp":0,"mp":0,"atk":0,"def":0,"agl":40,"wis":0,"move":0}
...
var TalentBlossomStats = {"hp":0,"mp":0,"atk":0,"def":0,"agl":20,"wis":0,"move":0};
...
function applyStats() {
    ...
    $('#MP').text(Math.floor((538 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].mp : 0)) * ((AwkStatsUp[Awk] ? AwkStatsUp[Awk].mp : 0) + (ElementalStatsUp[Elemental] ? ElementalStatsUp[Elemental].mp : 0))));
    $('#ATK').text(Math.floor((205 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].atk : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].atk : 0)));
    $('#DEF').text(Math.floor((408 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].def : 0)) * ((AwkStatsUp[Awk] ? AwkStatsUp[Awk].def : 0) + (ElementalStatsUp[Elemental] ? ElementalStatsUp[Elemental].def : 0))));
