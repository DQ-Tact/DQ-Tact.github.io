...
var TalentBlossomStats = {"code":null,"hp":100,"mp":30,"atk":15,"def":25,"agl":15,"wis":15,"move":0,"stype":null};
var TalentBlossomStatsUpNode = {"code":null,"hp":0.05,"mp":0.05,"atk":0.05,"def":0.05,"agl":0.05,"wis":0.05,"move":0,"stype":null};
...
function applyStats() {
    $('#HP').text(Math.floor((915 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].hp : 0) + (FullTalent ? TalentBlossomStats.hp : 0)) * ((AwkStatsUp[Awk] ? AwkStatsUp[Awk].hp : 0) + (FullTalent ? TalentBlossomStatsUpNode.hp : 0) + (ElementalStatsUp[Elemental] ? ElementalStatsUp[Elemental].hp : 0))));
    $('#MP').text(Math.floor((380 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].mp : 0) + (FullTalent ? TalentBlossomStats.mp : 0)) * ((AwkStatsUp[Awk] ? AwkStatsUp[Awk].mp : 0) + (FullTalent ? TalentBlossomStatsUpNode.mp : 0) + (ElementalStatsUp[Elemental] ? ElementalStatsUp[Elemental].mp : 0))));
    $('#ATK').text(Math.floor((471 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].atk : 0) + (FullTalent ? TalentBlossomStats.atk : 0)) * ((AwkStatsUp[Awk] ? AwkStatsUp[Awk].atk : 0) + (FullTalent ? TalentBlossomStatsUpNode.atk : 0))));
    $('#DEF').text(Math.floor((290 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].def : 0) + (FullTalent ? TalentBlossomStats.def : 0)) * ((AwkStatsUp[Awk] ? AwkStatsUp[Awk].def : 0) + (FullTalent ? TalentBlossomStatsUpNode.def : 0) + (ElementalStatsUp[Elemental] ? ElementalStatsUp[Elemental].def : 0))));
    $('#AGL').text(Math.floor((364 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].agl : 0) + (FullTalent ? TalentBlossomStats.agl : 0)) * ((AwkStatsUp[Awk] ? AwkStatsUp[Awk].agl : 0) + (FullTalent ? TalentBlossomStatsUpNode.agl : 0))));
    $('#WIS').text(Math.floor((407 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].wis : 0) + (FullTalent ? TalentBlossomStats.wis : 0)) * ((AwkStatsUp[Awk] ? AwkStatsUp[Awk].wis : 0) + (FullTalent ? TalentBlossomStatsUpNode.wis : 0))));
