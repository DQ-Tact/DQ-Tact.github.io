...
var TalentBlossomStats = {"hp":30,"mp":20,"atk":10,"def":95,"agl":10,"wis":10,"move":0};
var FullTalent = false;

function applyStats() {
    $('#HP').text(Math.floor((777 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].hp : 0) + (FullTalent ? TalentBlossomStats.hp : 0)) * ((AwkStatsUp[Awk] ? AwkStatsUp[Awk].hp : 0) + (ElementalStatsUp[Elemental] ? ElementalStatsUp[Elemental].hp : 0))));
