...
var TalentBlossomStats = {"code":null,"hp":130,"mp":30,"atk":15,"def":25,"agl":15,"wis":15,"move":0,"stype":null};
...
function applyStats() {
    ...
    $('#ATK').text(Math.floor((240 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].atk : 0) + (FullTalent ? TalentBlossomStats.atk : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].atk : 0)));
    ...
    $('#WIS').text(Math.floor((375 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].wis : 0) + (FullTalent ? TalentBlossomStats.wis : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].wis : 0)));
