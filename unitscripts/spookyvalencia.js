...
var TalentBlossomStats = {"hp":130,"mp":30,"atk":15,"def":25,"agl":15,"wis":15,"move":0};
...
function applyStats() {
    ...
    $('#DEF').text(Math.floor((334 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].def : 0) + (FullTalent ? TalentBlossomStats.def : 0)) * ((AwkStatsUp[Awk] ? AwkStatsUp[Awk].def : 0) + (ElementalStatsUp[Elemental] ? ElementalStatsUp[Elemental].def : 0))));
