...
var TalentBlossomStats = {"hp":150,"mp":0,"atk":25,"def":25,"agl":25,"wis":25,"move":0};
var FullTalent = false;

function applyStats() {
    $('#HP').text(Math.floor((1210 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].hp : 0) + (FullTalent ? TalentBlossomStats.hp : 0)) * ((AwkStatsUp[Awk] ? AwkStatsUp[Awk].hp : 0) + (ElementalStatsUp[Elemental] ? ElementalStatsUp[Elemental].hp : 0))));
