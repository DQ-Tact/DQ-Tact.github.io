...
var TalentBlossomStats = {"hp":100,"mp":30,"atk":15,"def":25,"agl":55,"wis":15,"move":1};
var FullTalent = false;

function applyStats() {
    $('#HP').text(Math.floor((990 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].hp : 0) + (FullTalent ? TalentBlossomStats.hp : 0)) * ((AwkStatsUp[Awk] ? AwkStatsUp[Awk].hp : 0) + (ElementalStatsUp[Elemental] ? ElementalStatsUp[Elemental].hp : 0))));
