...
var TalentBlossomStats = {"code":null,"hp":60,"mp":20,"atk":10,"def":15,"agl":10,"wis":10,"move":1,"stype":null};
...
function applyStats() {
    ...
    $('#ATK').text(Math.floor((234 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].atk : 0) + (FullTalent ? TalentBlossomStats.atk : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].atk : 0)));
    ...
    $('#AGL').text(Math.floor((236 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].agl : 0) + (FullTalent ? TalentBlossomStats.agl : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].agl : 0)));
