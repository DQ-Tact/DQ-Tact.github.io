...
var AwkStatBuff =   {"5":{"code":null,"hp":100,"mp":0,"atk":0,"def":0,"agl":0,"wis":0,"move":0,"stype":null}
                    ,"4":{"code":null,"hp":100,"mp":0,"atk":0,"def":0,"agl":0,"wis":0,"move":0,"stype":null}
                    ,"3":{"code":null,"hp":100,"mp":0,"atk":0,"def":0,"agl":0,"wis":0,"move":0,"stype":null}
...
function applyStats() {
    $('#HP').text(Math.floor((1004 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].hp : 0)) * ((AwkStatsUp[Awk] ? AwkStatsUp[Awk].hp : 0) + (ElementalStatsUp[Elemental] ? ElementalStatsUp[Elemental].hp : 0))));
    ...
    $('#AGL').text(Math.floor((378 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].agl : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].agl : 0)));
    $('#WIS').text(Math.floor((137 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].wis : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].wis : 0)));
