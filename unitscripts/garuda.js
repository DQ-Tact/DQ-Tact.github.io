...
var AwkStatBuff =   {"5":{"code":null,"hp":0,"mp":0,"atk":0,"def":0,"agl":40,"wis":0,"move":0,"stype":null}
                    ,"4":{"code":null,"hp":0,"mp":0,"atk":0,"def":0,"agl":20,"wis":0,"move":0,"stype":null}
                    ,"3":{"code":null,"hp":0,"mp":0,"atk":0,"def":0,"agl":20,"wis":0,"move":0,"stype":null}
...
function applyStats() {
    $('#HP').text(Math.floor((740 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].hp : 0)) * ((AwkStatsUp[Awk] ? AwkStatsUp[Awk].hp : 0) + (ElementalStatsUp[Elemental] ? ElementalStatsUp[Elemental].hp : 0))));
    $('#MP').text(Math.floor((317 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].mp : 0)) * ((AwkStatsUp[Awk] ? AwkStatsUp[Awk].mp : 0)+ (ElementalStatsUp[Elemental] ? ElementalStatsUp[Elemental].mp : 0))));
    ...
    $('#WIS').text(Math.floor((211 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].wis : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].wis : 0)));
