...
function applyStats() {
    $('#HP').text(Math.floor((950 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].hp : 0) + (FullTalent ? TalentBlossomStats.hp : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].hp : 0)));
    ...
    $('#DEF').text(Math.floor((398 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].def : 0) + (FullTalent ? TalentBlossomStats.def : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].def : 0)));
