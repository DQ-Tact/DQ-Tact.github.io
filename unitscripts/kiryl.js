...
var AwkStatBuff =   {"5":{"hp":100,"mp":0,"atk":0,"def":0,"agl":0,"wis":0,"move":0}
                    ,"4":{"hp":50,"mp":0,"atk":0,"def":0,"agl":0,"wis":0,"move":0}
                    ,"3":{"hp":50,"mp":0,"atk":0,"def":0,"agl":0,"wis":0,"move":0}
...
var TalentBlossomStats = {"hp":100,"mp":30,"atk":15,"def":50,"agl":15,"wis":45,"move":1};
var FullTalent = false;

function applyStats() {
    $('#HP').text(Math.floor((929 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].hp : 0) + (FullTalent ? TalentBlossomStats.hp : 0)) * ((AwkStatsUp[Awk] ? AwkStatsUp[Awk].hp : 0) + (ElementalStatsUp[Elemental] ? ElementalStatsUp[Elemental].hp : 0))));
    ...
    $('#DEF').text(Math.floor((295 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].def : 0) + (FullTalent ? TalentBlossomStats.def : 0)) * ((AwkStatsUp[Awk] ? AwkStatsUp[Awk].def : 0) + (ElementalStatsUp[Elemental] ? ElementalStatsUp[Elemental].def : 0))));
    $('#AGL').text(Math.floor((310 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].agl : 0) + (FullTalent ? TalentBlossomStats.agl : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].agl : 0)));
    $('#WIS').text(Math.floor((352 + (AwkStatBuff[Awk] ? AwkStatBuff[Awk].wis : 0) + (FullTalent ? TalentBlossomStats.wis : 0)) * (AwkStatsUp[Awk] ? AwkStatsUp[Awk].wis : 0)));
